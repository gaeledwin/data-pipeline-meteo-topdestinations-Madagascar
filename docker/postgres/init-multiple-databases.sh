...
# Grant schema creation rights to ELT user
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" <<-EOSQL
    GRANT CREATE ON DATABASE $ETL_DATABASE_NAME TO $ETL_DATABASE_USERNAME;
    \c $ETL_DATABASE_NAME
    GRANT ALL ON SCHEMA public TO $ETL_DATABASE_USERNAME;
EOSQL
